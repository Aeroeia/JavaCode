...
- 通过本地的或者Redisson的延迟消息队列进行（底层是二叉堆+阻塞队列方式 按照任务到期时间排序然后await等待 如果到期则唤醒线程执行任务）
...
- 这种方式只是让消费者按配置的速度进行消费 有可能延迟任务会被提前拉取 解决办法是在java中再用一层延迟队列 等到时后再放到真正的topic中
...
- 有原生的延迟队列和死信队列（达到最大次数（16）后传入 topic：%DLQ%<consumerGroup>） 延迟队列：==定期扫描将消息从延迟索引队列移动到真正的 Topic/Queue==  

  > - Java中DelayQueue ： 通过二叉堆+阻塞队列方式 按照任务到期时间排序然后await等待 如果到期则唤醒线程执行任务
  >
  > - RocketMq则是定期扫描每个延迟级别的队头因为：一个 delayLevel 的消息延迟时间是**固定的**
  >    → queue 内的消息到期时间顺序与写入顺序一致 然后到期转发
...
2. 初始化
   ...
   > - 如果Bean实现了BeanFactoryAware接口的话，Spring将调用setBeanFactory()方法，将 容器实例传入
...
读取配置类 / XML
   ↓
注册 BeanDefinition（只是描述信息 Bean名、作用域、生命周期）
- 将读取到的配置信息解析成一个个 BeanDefinition 对象，并注册到 BeanFactory 中（通常保存在 DefaultListableBeanFactory 的一个 Map<String, BeanDefinition> 中）。
- 生产 Bean：
当开发者调用 BeanFactory.getBean("user") 时；
或者容器启动进行单例预加载时；
BeanFactory 会去查找名为 "user" 的 BeanDefinition。
- 存储位置： 创建好的单例 Bean 实例会被存放在 DefaultSingletonBeanRegistry 类中的一个 Map 里。
Map 的名字： singletonObjects。
数据结构： Map<String, Object> (Key 是 beanName, Value 是 Bean 实例)。
通俗叫法： 这就是我们要常听到的 “单例池” 或者 “一级缓存”。
   ↓
【BeanFactoryPostProcessor】生效（可以修改 BeanDefinition）
   ↓
实例化 Bean（new）（通过反射推断构造方法）
[是否存在 @Autowired]
        ├── 是 → 解析注入最明确的构造器
        │        ├── 唯一成功 → 使用它
        │        └── 多个满足 → 冲突报错
...
依赖注入（@Autowired 等）
 ...
 Aware 回调
 	 ↓
【BeanPostProcessor.beforeInitialization】（Bean 初始化前 ）
   ↓
调用 @PostConstruct / init-method等初始化方法
   ↓
【BeanPostProcessor.afterInitialization】（Bean 初始化后 创建、初始化代理对象）
   ↓
Bean 可用
```
...
> ApplicationContext继承了BeanFactory 对BeanFactory进行了增强 BeanFactory主要聚焦于BeanFactory 聚焦 **Bean 的创建与管理** 而ApplicationContext = BeanFactory + 国际化 + 事件发布监听 + AOP 自动装配 + 环境/配置管理 + 资源加载 + 启动时预实例化 Bean

------

#### AOP

> AOP主流的实现主要有两个 分别是AspectJ、SpringAOP

**AspectJ**

- 在编译期进行增强 直接操作字节码 将定义的增强逻辑直接写入到类当中
- 只要编译被织入，任何地方调用这个方法都会执行增强后的逻辑 而不像Springaop那样 如果通过this.调用 会让代理方法失效

**SpringAOP**

- 只能增强SpringBean 基于动态代理

- 如果类实现了接口 则基于jdk动态代理 否则基于cglib动态代理

  > jdk动态代理基于当前接口通过反射生成Class对象
  >
  > cglib基于当前类生成一个增强类 通过ASM修改字节码 在元空间中添加代理类信息并生成对应Class对象

- 注解都是基于Aspct的（Before、Around、After、PointCut） 但是具体实现不同
...
> **在 Spring Boot 2.x 中：**
> 该属性默认值被设置为 true。这意味着 AOP 代理是基于类（Target Class）生成的，而不是基于接口，因此使用 CGLIB。
>
> - 这样做的好处是：开发者在注入 Bean 时，可以直接使用**具体实现类**来接收注入，而不仅仅局限于接口，减少了 BeanNotOfRequiredTypeException 异常的发生，提升了开发体验
...
![image-20251121011541928](assets/image-20251121011541928.png)

REQUIRED 使用同一个事务；NESTED 只管自己局部

**事务失效**

![image-20251121014413111](assets/image-20251121014413111.png)

- 事务方法非public修饰
- 非事务方法调用事务方法
- 事务方法的异常被捕获了
- 事务异常类型不对 `rollbackfor`
- 事务传播行为不对
- Transational非Bean Transational是基于spring的 springbean才能生效
- 直接调用方法导致代理失效 需要通过代理类调用
- 方法为final 子类不能重写方法导致代理失效
- 多线程导致失效 是因为检查不到当前线程有事务存在 
...
发现通过@Lazy懒加载也可以解决 是因为懒加载会注入一个代理对象 从而打破循环依赖

------

#### Spring、SpringBoot、SpringMVC区别

![image-20251121021404115](assets/image-20251121021404115.png)

##### **SpringBoot**

- 例如引入 `spring-boot-starter-web` 会自动配置 Tomcat、Spring MVC、Jackson 等

- 通过 `spring.factories` 指定扫描的类：用于 **自动配置（Auto-Configuration）**，让 Spring Boot 在启动时自动加载指定类，而无需手动 `@Configuration` 或 `@Import`

##### 自动配置

![image-20251122234244766](assets/image-20251122234244766.png)

==自动装配是autowired注解 Spring就有 而自动配置是通过SPI机制扫描第三方jar包的META-INF目录下的配置文件并执行配置发现、Bean注册等操作 springboot才有== 

##### SpringMVC

![image-20251123000356325](assets/image-20251123000356325.png)
...
1. **注解解析（入口）**
   ...
   > 首先要有一个加了 `@SpringBootApplication` 注解的启动类。
   > 这个注解由 `@EnableAutoConfiguration`、`@SpringBootConfiguration`、`@ComponentScan` 组成。
   >
   > *   最为核心的是 `@EnableAutoConfiguration`，它配合 `@Import(AutoConfigurationImportSelector.class)`，通过 SPI 机制扫描第三方 jar 包 `META-INF` 目录下的配置文件（`spring.factories` 或 `.imports`）。
   > *   `@SpringBootConfiguration` 等同于 `@Configuration`，用于标志当前类为配置类。

2. **服务构建（New SpringApplication）**

   > 构建 `SpringApplication` 这个服务对象（在 run 方法执行之前）。
   >
   > 1.  将资源加载器、主方法类记录到内存中。
   > 2.  **推断 Web 应用类型**：逐一判断 classpath 下是否存在特定的类，从而确定是 Servlet（Tomcat环境）、WebFlux 还是 None。
   > 3.  **加载初始化器和监听器**：从 `spring.factories` 读取所有的 `ApplicationContextInitializer` 和 `ApplicationListener` 并保存到集合中（此时尚未执行，只是加载了实例）。

3. **环境准备（Environment）**

   > `run` 方法开始执行，根据不同的 Web 服务类型构造不同的环境对象（StandardServletEnvironment 等）。
   >
   > 1.  加载系统环境变量、JVM 系统属性。
   > 2.  **加载配置文件**：处理 `application.properties/yaml`，将这些配置加载到 `PropertySource` 内存集合中。
   > 3.  发布 `ApplicationEnvironmentPreparedEvent` 事件。
   >
   > *   **准确点：** 监听器是观察者模式，**同步串行**执行（除非标注了 `@Async`），确保环境准备好后才能进行后续步骤。

4. **容器创建与预处理（Context Create & Prepare）**

   > 创建并准备上下文（ApplicationContext）。
   >
   > 1.  **工厂创建**：根据 Web 类型创建对应的容器（如 `AnnotationConfigServletWebServerApplicationContext`），容器内部会自动创建 `DefaultListableBeanFactory`。
   > 2.  **执行初始化器**：回调在第 2 步加载的 `ApplicationContextInitializer`，对容器进行自定义设置。
   > 3.  **注册启动类**：将我们的 Main 启动类作为一个 `BeanDefinition` 注册到容器中（此时容器里只有这一张核心图纸）。
   >
   > *   **注意**：此时**还未开始**全量解析 `@ComponentScan` 和 `@Autowired`。

5. **容器刷新与填充（Refresh - 核心）**

   > 调用核心的 `refresh()` 方法，这是 Spring 的心脏。
   >
   > 1.  **解析配置与扫描**：执行 `BeanFactoryPostProcessor`（主要是 `ConfigurationClassPostProcessor`），解析 `@ComponentScan`、`@Import`、`@Bean`，并将 `@EnableAutoConfiguration` 导入的那些配置类全部转为 `BeanDefinition` 存入 BeanDefinitionMap。
   > 2.  **启动 Web 服务器**：执行 `onRefresh()` 方法，**创建并启动内置的 Tomcat/Jetty**。
   > 3.  **实例化单例 Bean**：遍历 BeanDefinitionMap，通过反射创建所有非懒加载的单例 Bean，并处理 `@Autowired` 依赖注入和生命周期回调（PostConstruct）。

6. **收尾与回调（After Refresh）**

   > 1.  发布 `ApplicationStartedEvent`（标志容器和 Web 服务器已启动）。
   > 2.  **执行 Runners**：回调所有的 `CommandLineRunner` 和 `ApplicationRunner` 接口（用于业务数据预热）。
   > 3.  发布 `ApplicationReadyEvent`（标志系统彻底就绪）。

##### **对比Spring**

> **1. 启动入口与控制权（最根本的区别）**
>
> *   **Spring (传统): ** **Tomcat 启动 Spring**
>     *   **入口：** 没有 `main` 方法。容器是 Tomcat/Jetty。
>     *   **流程：** 启动 Tomcat -> 加载 `web.xml` -> 触发 `ContextLoaderListener` -> **初始化 Spring 容器**。
>     *   **形态：** 必须打成 `war` 包，丢到外部容器运行。
>
> *   **Spring Boot: ** **Spring 启动 Tomcat**
>     *   **入口：** 标准的 Java `main` 方法。
>     *   **流程：** `SpringApplication.run()` -> 准备环境 -> 创建 Spring 容器 -> **在容器刷新过程中代码启动内嵌 Tomcat**。
>     *   **形态：** 可以打成可执行 `jar` 包，自包含环境。
>
> **2. 配置加载方式（图纸来源不同）**
>
> *   **Spring (传统): ** **显式配置 (Explicit)**
>     *   **做法：** 必须手动编写 XML 或 Java Config 类。
>     *   **核心：** 你写了 `@ComponentScan`，它才扫；你配置了 `InternalResourceViewResolver`，它才有视图解析器。**不写就没有。**
>
> *   **Spring Boot: ** **约定大于配置 + SPI (Implicit)**
>     *   **做法：** 依赖 `starter` 和 `@EnableAutoConfiguration`。
>     *   **核心：** **SPI 机制**（Step 1 中提到的）。即使你没写配置，只要 jar 包在，Boot 就会去 `META-INF` 里找默认配置并加载。**不写也能跑。**
>
> **3. 环境准备阶段（Environment）**
>
> *   **Spring (传统):**
>     *   `Environment` 对象相对简单，主要读取 `web.xml` 中的 `<context-param>` 和 JVM 系统属性。
>     *   通常不支持 YAML 文件，读取 `properties` 需要手动配置 `PropertyPlaceholderConfigurer`。
>
> *   **Spring Boot:**
>     *   **Step 3** 极其复杂。它构建了一个复合的 `Environment`。
>     *   支持 `application.yaml` / `properties`。
>     *   支持命令行参数（`--server.port=8080`）。
>     *   支持 Profile 多环境切换（dev/prod）。
>     *   **Banner 打印** 也是在这个阶段。
>
> **4. Web 容器的创建时机**
>
> *   **Spring (传统):**
>     *   Web 容器（Tomcat）早已启动，Spring 容器是作为 Web 容器的一个“插件”存在的。
>
> *   **Spring Boot:**
>     *   在 **Step 5 (refresh)** 的特定子阶段 `onRefresh()` 中。
>     *   Boot 检查到是 Web 环境，就会执行代码 `new Tomcat()`，配置端口、ContextPath，然后 `tomcat.start()`。
>     *   **DispatcherServlet** 是在这个时候被注册到内嵌 Tomcat 里的。
>
> **5. 扩展机制（监听器与Runner）**
>
> *   **Spring (传统):**
>     *   虽然也有 `ApplicationListener`，但通常用于业务逻辑。
>     *   没有 `CommandLineRunner` 这种明确的“启动后钩子”。
>
> *   **Spring Boot:**
>     *   **Step 2** 加载的 8 个监听器和 7 个初始化器贯穿全程，处理日志、颜色、异常报告。
>     *   **Step 6** 专门执行 `Runners`，方便数据预热。
>
> ---
>
> **总结对比表**
>
> | 维度            | 传统 Spring 启动                                   | Spring Boot 启动                           |
> | :-------------- | :------------------------------------------------- | :----------------------------------------- |
> | **启动者**      | **外部 Tomcat** (web.xml 触发)                     | **Java Main 方法** (SpringApplication.run) |
> | **Web服务器**   | 容器存在于 Spring **之外**                         | 容器作为 Bean 存在于 Spring **之内**       |
> | **配置来源**    | 程序员手写 XML/JavaConfig                          | 手写 + **SPI 自动扫描 (META-INF)**         |
> | **配置文件**    | 需要手动加载 properties                            | 自动加载 application.yaml/properties       |
> | **Servlet注册** | 在 `web.xml` 或 `WebApplicationInitializer` 中配置 | 自动注册 DispatcherServlet                 |
> | **运行产物**    | .war 文件                                          | .jar 文件 (Fat Jar)                        |
>
> **一句话概括：**
> Spring 的启动是 **“填空题”**（容器给你了，你往里填组件）；
> Spring Boot 的启动是 **“造房子”**（从地基 Environment 开始，自己造柱子 Context，最后连门窗 Tomcat 都是自己安上去的）。

------

##### 上下文和监听器

**一、 7 个上下文初始化器 (ApplicationContextInitializer)**

**执行时机：** 容器（Context）被 new 出来后，但在 refresh()（加载 Bean）之前。
**核心作用：** 对还没开始干活的容器进行“结构性”的修改或属性注入。

| 序号 | 组件名称 (简称)                                        | 核心作用                                                     |
| ---- | ------------------------------------------------------ | ------------------------------------------------------------ |
| 1    | **DelegatingApplicationContextInitializer**            | **委派者**。<br>支持在 application.properties 中配置 context.initializer.classes，它负责把这些自定义的初始化器加载进来并执行。 |
| 2    | **ContextIdApplicationContextInitializer**             | **身份证**。<br>给容器设置唯一的 ID (ApplicationId)。格式通常是 application:profile:port，用于在微服务或云环境中区分不同的服务实例。 |
| 3    | **ConfigurationWarningsApplicationContextInitializer** | **安检员**。<br>检查常见的配置错误并发出警告。例如：如果你把包扫描路径 (@ComponentScan) 设置为了 org.springframework，它会警告你这样会扫描太慢且不安全。 |
| 4    | **ServerPortInfoApplicationContextInitializer**        | **端口暴露**。<br>在 Web 容器启动并确定端口（特别是随机端口）后，将端口号写入 Environment 的 local.server.port 属性中，方便后续获取。 |
| 5    | **SharedMetadataReaderFactoryContextInitializer**      | **加速器**。<br>创建一个共享的元数据读取工厂（CachingMetadataReaderFactory），用于缓存类的注解信息，提升 Spring 启动时的扫描速度。 |
| 6    | **ConditionEvaluationReportLoggingListener**           | **报告员**。<br>(注：它实现了 Initializer 接口注册自己)。它负责在启动失败或开启 Debug 模式时，打印那个著名的 **AutoConfiguration Report**（显示哪些自动配置生效了，哪些没生效）。 |
| 7    | **RSocketPortInfoApplicationContextInitializer**       | **RSocket支持**。<br>(通常在引入 RSocket 依赖时出现)。作用同第4点，只不过它是暴露 RSocket 服务的端口信息。 |

*(注：如果没有引入 RSocket，第 7 个有时会是 PropertySourceOrderingApplicationContextInitializer，用于调整属性源的优先级)*

**二、 8 个监听器 (ApplicationListener)**

**执行时机：** 贯穿整个启动生命周期（Starting -> EnvironmentPrepared -> ContextInitialized -> Started -> Ready/Failed）。
**核心作用：** 监听特定事件，执行对应的基础设施设置或清理工作。

| 序号 | 组件名称 (简称)                            | 核心作用                                                     |
| ---- | ------------------------------------------ | ------------------------------------------------------------ |
| 1    | **LoggingApplicationListener**             | **日志官 (最重要)**。<br>监听 EnvironmentPreparedEvent。根据配置初始化日志系统（Logback/Log4j2）。它必须最早工作，否则启动过程没有日志。 |
| 2    | **BackgroundPreinitializer**               | **预热工**。<br>监听 StartingEvent。启动一个后台线程，提前初始化一些耗时的组件（如消息转换器、验证器），利用多线程让主流程启动更快。 |
| 3    | **DelegatingApplicationListener**          | **委派者**。<br>监听 EnvironmentPreparedEvent。支持在配置文件中通过 context.listener.classes 配置自定义监听器，并将其注册到广播器中。 |
| 4    | **ParentContextCloserApplicationListener** | **父子联动**。<br>监听 ContextRefreshedEvent。如果当前容器是子容器，它会绑定父容器的关闭事件。父容器关闭时，子容器也会自动关闭。 |
| 5    | **ClearCachesApplicationListener**         | **清洁工**。<br>监听 ContextRefreshedEvent。在容器刷新完成后，清理 JDK 和 CGLIB 的反射缓存、类加载器缓存，释放内存。 |
| 6    | **FileEncodingApplicationListener**        | **编码检查**。<br>监听 EnvironmentPreparedEvent。检查 System.getProperty("file.encoding") 是否与 spring.mandatory-file-encoding 配置一致，不一致则报错。 |
| 7    | **AnsiOutputApplicationListener**          | **调色师**。<br>监听 EnvironmentPreparedEvent。根据配置 spring.output.ansi.enabled 决定控制台日志和 Banner 是否显示彩色。 |
| 8    | **ClasspathLoggingApplicationListener**    | **排错员**。<br>监听 ApplicationFailedEvent。平时不说话，只有在**启动失败**时，将当前的 Classpath 路径全部打印出来，帮助开发者排查是不是缺包或版本冲突。 |

- **ApplicationContextInitializer** 和 **ApplicationListener** 是 Spring 定义的 **标准（接口）**。
- 你之前看到的列表（如 ContextId..., Logging...）是 Spring Boot 官方写好的、符合这些标准的 **具体组件（实现类）**。
- Spring Boot 启动时，会通过 SPI 机制把这 7 个实现类和 8 个实现类都加载进来，转型为接口去调用它们。
