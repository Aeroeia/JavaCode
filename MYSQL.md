...
- 解析 SQL，通过解析器对 SQL 查询语句进行词法分析、语法分析，然后构建语法树，方便后续模块读取表名、字段、语句类型；==解析阶段就会进行**语法检查**，确保 SQL 文法正确，否则就报错==
- 执行 SQL：执行 SQL 共有三个阶段：
  - 预处理阶段：检查表或字段是否存在；将 `select *` 中的 `*` 符号扩展为表上的所有列。==这个阶段保证 SQL 能在当前数据库上下文中执行，不会引用不存在的对象。==
  - 优化阶段：基于查询成本的考虑， 选择查询成本最小的执行计划；==这个阶段主要是 **选择最优的执行计划**，因为同一个 SQL 可能有多种执行方式。==
...
- **索引结构**：==InnoDB 是聚簇索引，MyISAM 是非聚簇索引==。聚簇索引的文件存放在主键索引的叶子节点上，因此 InnoDB 必须要有主键，通过主键索引效率很高。但是辅助索引需要两次查询，先查询到主键，然后再通过主键查询到数据。因此，主键不应该过大，因为主键太大，其他索引也都会很大。而 MyISAM 是非聚簇索引，数据文件是分离的，索引保存的是数据文件的指针。主键索引和辅助索引是独立的。
  ...
  >
  > 1. **叶子节点存储整行数据**
  >    - 聚簇索引的 B+Tree 叶子节点，不仅存主键，还存整行的数据。
  >    - 因此通过主键查询时非常快。
  > 2. **一张表只能有一个聚簇索引**
  >    - 因为数据只能按照一种顺序存储。
  >    - 通常就是 **主键**，如果没有主键，InnoDB 会选择一个唯一非空索引，如果没有，再生成一个隐藏的 row_id。
  > 3. **辅助索引（Secondary Index）存储的是主键值**
  >    - 辅助索引的叶子节点不存整行数据，而是存储主键值。
  >    - 所以用辅助索引查数据时，需要回到聚簇索引里再查一次，这叫 **回表**。

  > **PRIMARY KEY 永远是聚簇索引（如果存在的话）**。
  >
  > 如果有 **第一个唯一且非空的索引**（Unique + NOT NULL），它会成为聚簇索引。
  >
  > - 此时即使你有一个 `id` 字段当普通主键（可能不是表定义的 PRIMARY KEY，而是后来加的），它就成了 **辅助索引**。
  >
  > 如果没有主键，也没有唯一非空索引 → InnoDB 会自动生成一个 **隐藏的 6 字节 row_id**，作为聚簇索引。
  >
  > - 这时你后来加的主键字段（如果只是普通索引）也是辅助索引。

  ==聚簇索引叶子节点就是数据页，辅助索引叶子节点只记录定位信息，要回表才能取完整数据。==

- **锁粒度**：InnoDB 最小的锁粒度是行锁，MyISAM 最小的锁粒度是表锁。一个更新语句会锁住整张表，导致其他查询和更新都会被阻塞，因此并发访问受限。

- **count 的效率**：InnoDB 不保存表的具体行数，执行 select count(*) from table 时需要全表扫描。而MyISAM 用一个变量保存了整个表的行数，执行上述语句时只需要读出该变量即可，速度很快。==(关键在于事务)==

> ### 为什么 MyISAM 即使能定位，也不能锁？
>
> 因为“知道位置 ≠ 能控制访问”。
>  锁行的核心在于**引擎层要有锁表（lock manager）和事务上下文**。
>
> MyISAM 没有这些机制：
>
> 1. 它的 offset 是个“物理位置”，不是逻辑 ID；
>    - 如果其他线程删除或重写该行，新数据可能重用这个 offset；
>    - 这意味着锁住的物理位置不再对应同一条逻辑记录。
> 2. 它没有事务管理器来维护谁持有锁；
>    - 没法检测死锁；
>    - 没法回滚；
>    - 没法记录锁依赖关系。
...
- t_order.frm ，t_order 的==表结构会保存在这个文件==。在 MySQL 中建立一张表都会生成一个.frm 文件，该文件是用来保存每个表的元数据信息的，主要包含表结构定义。
- t_order.ibd，t_order 的==表数据会保存在这个文件==。表数据既可以存在共享表空间文件（文件名：ibdata1）里，也可以存放在独占表空间文件（文件名：表名字.ibd）。这个行为是由参数 innodb_file_per_table 控制的，若设置了参数 innodb_file_per_table 为 1，则会将存储的数据、索引等信息单独存储在一个独占表空间，从 MySQL 5.6.6 版本开始，它的默认值就是 1 了，==因此从这个版本之后， MySQL 中每一张表的数据都存放在一个独立的 .ibd 文件。==

> **MySQL 5.6.6+**：
>
> - `.ibd` 文件存放 **数据 + 索引**。
> - 表结构还是 `.frm` 文件保存，不在 .ibd 里。
>
> **MySQL 8.0+**：
>
> - `.ibd` 文件里才真正存放 **表结构 + 数据 + 索引**。
...
**这句话的意思是：只取 `email` 字段的前 20 个字符作为索引键。**
...
因此，使用联合索引时，存在**最左匹配原则**，也就是按照==最左优先的方式进行索引的匹配==。在使用联合索引进行查询的时候，如果不遵循「最左匹配原则」，联合索引会失效，这样就无法利用到索引快速查询的特性了。
...
| ------------------- | ------------------------------- | --------------------------------------------- |
| `WHERE A=?`         | ✅ 可以                          | 使用最左列 A                                  |
| `WHERE A=? AND B=?` | ✅ 可以                          | 最左前缀 A+B                                  |
| `WHERE A=? AND C=?` | ✅ 可以查 A，但不能利用 C 做索引 | C 不连续，索引只能用最左前缀 A                |
| `WHERE B=?`         | ❌ 不行                          | 跳过最左列 A → 索引失效（只能全表扫描或回表） |
| `WHERE C=?`         | ❌ 不行                          | 同上                                          |
...
- 字段具有唯一性，且不能为空的特性
- 字段最好的是有==递增的趋势的==，如果字段的值是随机无序的，可能会引发页分裂的问题，造型性能影响。
...
而且，UUID 太占用内存。每个 UUID 由 36 个字符(16进制)组成，在字符串进行比较时，需要从前往后比较，字符串越长，性能越差。另外字符串越长，==占用的内存越大==，由于页的大小是固定的，这样一个页上能存放的关键字数量就会越少，这样最终就会导致索引树的高度越大，在索引搜索的时候，发生的磁盘 IO 次数越多，性能越差
...
![image-20250923151309662](assets/image-20250923151309662.png)**聚簇索引**

- 非叶子节点存区间索引，叶子节点存储实际数据 都只使用一个数组结构

**辅助索引**

- 非叶子节点存储数据，叶子节点存储主键索引
...
- B+树的叶子节点使用==链表相连==，便于范围查询和顺序访问；B树的叶子节点没有链表连接。
...
- B+ 树叶子节点之间用==链表连接了起来，有利于范围查询==，而 B 树要实现范围查询，因此只能通过树的遍历来完成范围查询，这会涉及多个节点的磁盘 I/O 操作，范围查询效率不如 B+ 树。

------

## MySQL为什么用B+树结构？和其他结构比的优点？

- **B+Tree vs B Tree：**B+Tree 只在叶子节点存储数据，而 B 树 的非叶子节点也要存储数据，所以 B+Tree 的单个节点的数据量更小，==在相同的磁盘 I/O 次数下，就能查询更多的节点==。另外，B+Tree 叶子节点采用的是双链表连接，适合 MySQL 中常见的基于范围的==顺序查找==，而 B 树无法做到这一点。
...
- **B+Tree vs 跳表：**B+树的高度在3层时存储的数据可能已达千万级别，但对于跳表而言同样去维护千万的数据量那么所造成的跳表层数过高而导致的磁盘io次数增多，也就是使用B+树在存储同样的数据下磁盘io次数更少。
...
- MySQL 在遇到==字符串和数字比较==的时候，会自动把字符串转为数字，然后再进行比较。如果字符串是索引列，而条件语句中的输入参数是数字的话，那么索引列会发生隐式类型转换，由于隐式类型转换是==通过 CAST 函数==实现的，等同于对索引列使用了函数，所以就会导致索引失效。
...
==每次对表里字段做增/改都会留下版本号 然后执行事务时会根据版本号==

- **在 undo log 里保存一份旧值；**
- **在行的 `DB_TRX_ID` 里写入当前事务的 ID。**

这样这行就有了多个版本：最新版本在数据页，旧版本链在 undo log 中。

> 行 `price=100`，事务 ID=10；
>
> 事务 11 把它改成 200，写 undo log 保存 `100`；
>
> 事务 12 开始时，Read View 看到事务 11 还没提交 → 它读到的 `price=100`（去 undo log 取）；
>
> 事务 11 提交后，新事务 13 开始就会读到 `price=200`。

| 特性       | Undo Log                       | Redo Log                                 |
| ---------- | ------------------------------ | ---------------------------------------- |
| 解决问题   | 回滚、MVCC（旧版本）           | 崩溃恢复（保证提交数据不丢）             |
| 写入时机   | 修改前，保存旧值               | 修改后，记录新值的“动作”(取决于参数策略) |
| 存储位置   | Undo tablespace                | Redo log file（ib_logfile）              |
| 生命周期   | 事务提交或过期版本清理后可删除 | 循环写，写满后覆盖                       |
| 关联的ACID | 原子性（A）、隔离性（I）       | 持久性（D）                              |

**redolog：**

> **成功标识** = redo log 中的 commit entry；
>
> **失败标识** = 其实没有显式写“失败”，只是因为没写 commit entry，所以被视为失败；
>
> **重启恢复逻辑**：
>
> - 有 commit entry → redo；
> - 没有 commit entry → rollback（靠 undo log）。
...
![image-20250925220051896](assets/image-20250925220051896.png)
...
## 各隔离级别和 Read View(一致性视图) 的关系

==read view决定如何找undolog数据==

**只在读已提交&重复读两种隔离级别中应用**

事务中读已提交每次select创建视图，而重复读只创建一次

1. **READ UNCOMMITTED**
   - 基本不使用 Read View。
   - 直接读“最新版本”，哪怕对方事务还没提交 → 可能脏读。
2. **READ COMMITTED (RC)**
   - **每次查询都会生成一个新的 Read View**。
   - 所以事务里的每条 `SELECT` 都能看到 **最新已提交的数据**。
   - 结果：同一个事务里，多次查询可能读到不同结果（不可重复读）。
3. **REPEATABLE READ (RR, MySQL 默认)**
   - **事务第一次执行 SELECT 时生成一个 Read View**，整个事务期间都复用这个视图。
   - 所以多次查询同一行数据，结果保持一致（可重复读）。
   - 结果：即使别的事务提交了更新，这个事务也看不到，除非自己结束。
4. **SERIALIZABLE**
   - 不靠 Read View，而是直接用锁来保证串行化。
   - 查询时会加锁，避免并发修改。

![image-20250926015358996](assets/image-20250926015358996.png)
...
![image-20251103001433525](assets/image-20251103001433525.png)
...
> ## 1️⃣ 串行化隔离级别的实现方式
>
> - MySQL 的事务隔离级别主要依赖 **MVCC + 锁**。
> - 在 **SERIALIZABLE** 模式下，InnoDB 会把普通的 `SELECT` 语句也转成加锁读（即 `SELECT ... LOCK IN SHARE MODE`）。
> - 所以它不只是用行级锁，还会更“保守”。
>
> ------
>
> ## 2️⃣ 加的是什么锁？
>
> - 如果是 **带条件的 SELECT**（`where` 精确匹配索引）：
>   - InnoDB 会加 **共享锁 (S 锁)** 在涉及的行上，保证别人不能改这行数据。
>   - 其他事务如果要更新这些行，会被阻塞。
> - 如果是 **范围查询**（`BETWEEN`、`>`、`<` 等）：
>   - InnoDB 会加 **间隙锁 (Gap Lock)** 或 **Next-Key Lock**，锁定范围，防止“幻读”（别人插入新行）。
> - 如果是 **不走索引的全表扫描**：
>   - 有可能退化成 **表级锁**，影响更大。
>
> ------
...
>
> - 串行化隔离级别下：
>   - **读操作**：会加共享锁 (S 锁)，所以多个事务同时读同一行是允许的（共享读 ✅）。
>   - **写操作**：需要独占锁 (X 锁)，会和 S 锁互斥 → 这时读写会互相阻塞。
>
> 👉 这就是为什么 SERIALIZABLE 很保守：**读也可能被写阻塞，写也可能被读阻塞**。
>
> ------
>
> ## 4️⃣ 总结
>
> - 串行化隔离级别下：
>   - 加的是 **行级锁 + 间隙锁/Next-Key 锁**，避免幻读。
>   - 是支持 **共享读** 的（多个事务可以一起读），但会阻塞写操作。
> - 缺点：并发性能很差，通常只在极高一致性要求的场景才用。
...
- 如果记录的 ==trx_id 值小于 Read View 中的 min_trx_id== 值，表示这个版本的记录是在创建 Read View **前==**已经提交的事务==生成的，所以该版本的记录对当前事务**可见**。

- 如果记录的 ==trx_id 值大于等于 Read View 中的 max_trx_id 值==，表示这个版本的记录是在创建 ==Read View **后**才启动的事务生成==的，所以该版本的记录对当前事务**不可见**。
...
- **表级锁**：MySQL 里面表级别的锁有这几种：
  ...
  - 表锁：通过lock tables 语句可以对表加表锁，表锁除了会限制别的线程的读写外，也会限制本线程接下来的读写操作。==可以是读锁或者写锁==
...
- 间隙锁，只存在于可重复读隔离级别，目的是为了解决可重复读隔离级别下幻读的现象==锁定的是 **两个索引值之间的间隙**，防止别人往这个间隙里插入数据。==

  ```sql
  SELECT * FROM t WHERE id > 10 AND id < 20 FOR UPDATE;
  ```

- Next-Key Lock 称为临键锁，是 Record Lock(行锁) + Gap Lock(间隙锁) 的组合，锁定一个范围，并且锁定记录本身。==锁定的是 **一条记录本身 + 它前面的间隙**。==

  ```sql
  SELECT * FROM t WHERE id = 20 FOR UPDATE;
  ```

假设 user 表有主键 id：1, 5, 10

```
SELECT * FROM user WHERE id > 5 FOR UPDATE;
```

- InnoDB 会对 `id=10` 加 **临键锁**（锁住 5~10 区间 + 10 记录）。
- 同时还会对 (10, +∞) 区间加间隙锁，防止插入新记录。

==如果 where 条件没用到索引（全表扫描），InnoDB 会对整个表加 **间隙锁 + 记录锁（相当于表锁行为）**，因为它没法精确控制行。==
...
- redo log 重做日志，是 Innodb 存储引擎层生成的日志，实现了事务中的**持久性**，主要**用于掉电等故障恢复**；
  ...
  这些策略是==针对redolog而不是数据==

  > **刷盘到 redo log 文件（redo log file）**
  >
  > - 是否马上刷盘，取决于参数 `innodb_flush_log_at_trx_commit`：
  >   - `1`（默认）：**事务提交时必须 fsync 到磁盘**（最安全，最慢）。
  >   - `2`：事务提交时只写到 OS 缓冲，稍后 fsync（可能丢数据）。
  >   - `0`：事务提交时只写 redo log buffer，由后台线程异步写盘（性能高，风险大）

  **0（innodb_flush_log_at_trx_commit=0）**

  - 事务提交时：
     **只写 redo log buffer（在 MySQL 进程内存），连 OS 缓存都不写。**
  - 后台线程：
     每秒把 redo log buffer 写入 OS 缓存并 `fsync` 到磁盘。
  - 风险：
     MySQL 崩溃会丢失最多 **1 秒的事务日志**（因为它只在内存）。

  **2（innodb_flush_log_at_trx_commit=2）**

  - 事务提交时：
     把 redo log buffer **写到 OS Page Cache**（内核缓存），但 **不 fsync**。
  - 后台线程：
     每秒执行一次 `fsync` 把 OS 缓存数据落到磁盘。
  - 风险：
     MySQL 崩溃不会丢（OS 缓存还在），但 **操作系统崩溃/掉电会丢失 1 秒的数据**。

  | 策略  | 提交时动作                         | 后台线程       | 崩溃丢数据情况                        |
  | ----- | ---------------------------------- | -------------- | ------------------------------------- |
  | **0** | 写到 MySQL 内存（redo log buffer） | 每秒写 + fsync | MySQL crash 丢 1 秒；OS crash 丢 1 秒 |
  | **2** | 写到 OS 缓存                       | 每秒 fsync     | MySQL crash 不丢；OS crash 丢 1 秒    |
...
MySQL 在==完成一条更新操作==后，Server 层还会生成一条 binlog，等之后事务提交的时候，会将该事物执行过程中产生的所有 binlog 统一写 入 binlog 文件，binlog 是 MySQL 的 Server 层实现的日志，所有存储引擎都可以使用。
...
- MIXED：包含了 STATEMENT 和 ROW 模式，它会==根据不同的情况==自动使用 ROW 模式和 STATEMENT 模式；
...
- **写 redo log buffer 就判定写入成功**，指的是 InnoDB 内部认为“事务的修改已经被记录（在内存日志中），逻辑上不会丢”。

- **真正是否安全持久，要看 redo log 是否刷到磁盘**，这就是 `innodb_flush_log_at_trx_commit` 策略控制的点。
...
2. Redo log的顺序写入：redo log采用==追加写入==的方式，将redo日志记录追加到文件末尾，而不是随机写入。这样可以减少磁盘的随机I/O操作，提高写入性能。
3. Checkpoint机制：MySQL会定期将内存中的数据刷新到磁盘，同时将最新的LSN（Log Sequence Number）记录到磁盘中，这个LSN可以==确保redo log中的操作是按顺序执行==的。在恢复数据时，系统会根据LSN来确定从哪个位置开始应用redo log。
...
> **时刻 A：redo log 写 prepare 成功，但 binlog 还没写入磁盘 → 崩溃**
>
> - 重启时，InnoDB 扫描 redo log，发现这个事务在 `prepare` 状态。
> - 拿着它的 **XID 去 binlog 查**，发现 binlog 里根本没有这条事务。
> - 说明事务没对外生效，于是回滚事务。
>
> **时刻 B：redo log 写 prepare 成功，binlog 也写入成功，但 redo log 还没改成 commit → 崩溃**
>
> - 重启时，InnoDB 扫描 redo log，还是发现 `prepare` 状态。
> - 去 binlog 查，发现 binlog 已经有对应的 XID。
> - 说明事务已经对外生效（别人已经可能读到/从库会复制到）。
> - 所以 InnoDB 会自动把 redo log 补上 `commit`，事务算提交成功。
...
## undolog、redolog、binlog写入时机(以 UPDATE 为例)

1. **生成 undo log**（保存旧值，支持回滚/MVCC）。
2. **更新内存中的数据页**（Buffer Pool），同时写一条修改记录到 **redo log buffer**。
3. **事务提交时进入两阶段提交**：
   - (1) **写 redo log（prepare 状态），持久化到磁盘**。
   - (2) **写 binlog，并持久化到磁盘**。
   - (3) **redo log 改为 commit 状态**（此时不一定 fsync，只要写到 OS 的 page cache 就行）。

------

## MySQL 的两大层次

1. **Server 层（MySQL Server 层）**
   - **职责**：SQL 解析、优化、执行计划生成、权限控制、事务管理、binlog 写入等。
   - **特点**：
     - 与存储引擎无关，负责整个数据库实例的调度。
     - 所有 SQL 都先经过 Server 层处理。
   - **包含**：
     - **SQL Parser**（解析 SQL）
     - **Optimizer**（优化器，生成执行计划）
     - **Executor（执行器）**，调用存储引擎接口执行具体操作
     - **Transaction Coordinator**（协调 redo log / undo log / binlog 写入）
2. **Storage Engine 层（存储引擎层，如 InnoDB）**
   - **职责**：存储数据、索引管理、Buffer Pool 缓存、Redo/Undo log、事务隔离、MVCC。
   - **特点**：
     - 只处理具体数据页的增删改查。
     - 对 SQL 语义不了解，只管物理数据操作。

------

## 为什么 binlog 在 Server 层？

- **binlog 是逻辑日志**（记录 SQL 或行修改），**作用是复制/备份**。
- **逻辑日志是 Server 层的事情**，因为：
  1. 它需要保证对外可见，**所有存储引擎都通用**。
     - 如果在存储引擎里写 binlog，其他引擎（MyISAM、Memory）就需要重复实现。
  2. binlog 必须在事务提交的协调下写入（两阶段提交）。
     - Server 层先调用存储引擎写 redo log/undo log，再写 binlog，保证一致性。
- **总结**：binlog 在 Server 层，是 Server 对事务的“对外广播”，存储引擎只负责物理数据的修改（redo/undo log）。
...
5. 至此，一条记录更新完了。
6. 在一条更新语句执行完成后，然后开始记录该语句对应的 binlog，此时记录的 binlog 会被==保存到 binlog cache==，并没有刷新到硬盘上的 binlog 文件，在事务提交时才会统一将该事务运行过程中的所有 binlog 刷新到硬盘。
...
因为 redolog 写入==磁盘是顺序写==，而 b+树里数据页写入磁盘是随机写，顺序写的性能会比随机写好，这样可以提升事务提交的效率。
...
当 InnoDB 启动崩溃恢复（crash recovery）时，对于每一个可能被 Redo 修改的页，会执行以下步骤：

1. **从磁盘读取该页到内存**。
2. **校验页的完整性**：
   - 检查页尾的 **checksum** 是否匹配页内容。
   - 检查页头的 **LSN**（Log Sequence Number）是否 ≤ 当前 Redo Log 的 LSN（确保页不是“未来”的）。
3. **决策**：
   - ✅ **如果页是完整的**（checksum 正确，LSN 合理）→ **应用 Redo Log**。
   - ❌ **如果页损坏了**（checksum 错误，或 LSN 异常）→ **InnoDB 认为该页不可信，拒绝在其上应用任何 Redo**！

> 🛑 **这是出于数据安全的保守设计**：在一个已知损坏的页上盲目应用 Redo，可能会导致更严重的数据错乱或数据库无法启动。 

1. **“未来页”**（Future Page）
   - 页 A 被修改，写入 LSN=1000。
   - 之后系统崩溃，Redo Log 只持久化到 LSN=800。
   - 重启后，如果直接使用 LSN=1000 的页，就相当于“看到了未来的数据”，违反了 WAL（Write-Ahead Logging）原则。
2. **“过期页”**（Stale Page）
   - 页 B 在内存中被多次修改（LSN=500 → 700 → 900），但只在 LSN=700 时刷过盘。
   - 崩溃后，磁盘页的 `page_lsn = 700`，但 Redo Log 中还有 LSN=701~900 的修改。
   - 恢复时需要重做 LSN=701~900 的 Redo，但必须确保**不重复重做 LSN≤700 的操作**。

==可能出现lSN错误 导致redolog无法匹配==
...
需要注意的是，这个操作并非原子操作，比如我操作系统写到第二个页的时候，Linux机器断电了，这时候就会出现问题了。造成”页数据损坏“。并且这种”页数据损坏“靠 redo日志是无法修复的(==Redo log 只记录变化，应用时必须依赖一个“完整可用的旧页”。一旦旧页因为 torn page 被破坏，redo log 就失效了==)。==但注意**4KB 写入是原子的**，不会出现“写了 2KB 就断电”的情况。==因为硬盘控制器在 sector 层面有 **校验 + 写入缓存 + 原子提交机制**。
...
https://www.bilibili.com/video/BV1eV4y1D7fs/?spm_id_from=333.337.search-card.all.click&vd_source=fd0b7243ae5b04e4d46b23923b639f5b
