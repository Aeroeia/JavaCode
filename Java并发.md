...
Java 底层会调用 ==pthread_create== 来创建线程，所以本质上 java 程序创建的线程，

就是和操作系统线程是一样的，是 1 对 1 的线程模型。
...
**ThreadLocalMap是ThreadLocal的内部类 然后Thread保存了ThreadLocalMap变量 通过ThreadLocal.set时候先从当前线程获取Map 然后如果为空则新建ThreadLocalMap对象赋值给Thread中的threads变量 然后ThreadLocal.get是也是从线程获取Map然后获取到对应的ThreadLocal再取值 这样就保证了线程隔离**
...
> **互斥**：某资源同一时间只能被一个线程占有。
>
> **请求与保持**：一个线程持有资源 A，又去请求资源 B，不释放 A。
>
> **不可剥夺**：线程拿到的资源不能被强制抢走。
>
