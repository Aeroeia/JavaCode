...
- **基于类的代理**（CGLIB动态代理）： CGLIB（Code Generation Library）是一个强大的高性能的代码生成库，它可以在运行时动态生成一个目标类的子类。CGLIB代理不需要目标类实现接口，而是通过继承的方式创建代理类。因此，如果目标对象没有实现任何接口，可以使用CGLIB来创建动态代理。==**Byte Buddy**逐渐取代==
...
反射机制是指程序在运行状态下，对于任意一个类，都能够==获取这个类的所有属性和方法==；对于任意一个对象，都能够调用它的==任意属性和方法==。也就是说，Java 反射允许在运行时获取类的信息并动态操作对象，即使在编译时不知道具体的类也能实现。
...
- `earlySingletonObjects` (二级缓存)：存放的是提前==暴露的 Bean 的原始对象引用 或 早期代理对象引用==，==专门用来处理循环依赖==。==当一个 Bean 还在创建过程中（尚未完成属性填充和初始化）==，但它的引用需要被注入到另一个 Bean 时，就暂时放在这里。此时 Bean 已实例化（调用了构造函数），但属性尚未填充，初始化方法尚未执行，它可能是一个原始对象，也可能是一个为了解决 AOP 代理问题而提前生成的代理对象。
- `singletonFactories` (三级缓存)：存放的是 Bean 的 `ObjectFactory` 工厂对象。，这是==解决循环依赖和 AOP 代理协同工作的关键==。==当 Bean 被实例化后（刚调完构造函数），Spring 会创建一个 `ObjectFactory` 并将其放入三级缓存。==这个工厂的 `getObject()` 方法负责返回该 Bean 的早期引用（可能是原始对象，也可能是提前生成的代理对象），当检测到循环依赖需要注入一个尚未完全初始化的 Bean 时，就会调用这个工厂来获取==早期引用==。
...
- **二级缓存**（`earlySingletonObjects`）临时存储已确定的早期引用，避免重复生成代理； ==(保证单例)==
...
==**二级缓存升级时机**：当其他 Bean（如 `B`）需要注入当前 Bean（如 `A`）时，`A`的早期引用会从三级缓存升级到二级缓存。==
...
2. **二级缓存（Early Singleton Objects）**：这同样是一个Map类型的缓存，存储的是早期的bean引用，即==已经实例化但还未完全初始化的bean==。这些bean已经被实例化，但是可能还没有进行属性注入等操作。这个缓存在`DefaultSingletonBeanRegistry`类中的`earlySingletonObjects`属性中。
3. **三级缓存（Singleton Factories）**：这也是一个Map类型的缓存，存储的是==ObjectFactory==对象，这些对象==可以生成早期的bean引用==。当一个bean正在创建过程中，如果它被其他bean依赖，那么这个正在创建的bean就会通过这个ObjectFactory来创建一个早期引用，从而解决循环依赖的问题。这个缓存在`DefaultSingletonBeanRegistry`类中的`singletonFactories`属性中。





**分为有aop 无aop 创建代理时机分类 二级缓存：多个循环依赖发生时**

**为什么不能一开始就检查代理？ 如果所有 Bean 在实例化时都检查是否需要代理？**

​	**正常情况**：A 在初始化阶段才被 `AbstractAutoProxyCreator` 判断是否要代理。

​	**循环依赖情况**：当 B 需要 A 时，会从三级缓存触发 `getEarlyBeanReference()`，此时就会调用 `AbstractAutoProxyCreator` 提前判断并生成代理对象。	

- AOP 代理是通过 `AbstractAutoProxyCreator` 这样的 **BeanPostProcessor** 来决定的。
- `BeanPostProcessor` 的调用时机是在 **Bean 实例化之后、初始化之前**。
- 也就是说，Spring 在最早 `doCreateBean` 阶段还没法知道「这个 Bean 需不需要代理」。

👉 如果你在实例化的时候就生成代理，会跳过 `BeanPostProcessor` 的逻辑，导致很多 AOP/事务/增强功能全都失效。

**如果改为在 创建 Bean 时检查是否需要代理**

看似可以减少一次缓存穿透，但会带来几个问题：

1. **依赖注入时机错乱**
   - 如果在实例化后立刻代理，注入属性时可能打到代理对象身上。
   - 代理对象里一般持有目标对象的引用（比如 JDK 动态代理的 `InvocationHandler`，CGLIB 的 `target` 字段），此时目标对象可能还没注入完依赖，调用方法就会 NPE 或出错。
2. **BeanPostProcessor 的时机失效**
   - 很多 BPP 需要在属性注入后才能决定是否增强（例如事务增强器要看 `@Transactional` 是否生效）。
   - 如果在实例化阶段就代理，后置处理器还没跑到就失效了。
3. **循环依赖更麻烦**
   - 在三级缓存里，Spring 放的是“早期暴露对象”，它可能是原始对象，也可能包了一层代理工厂。
   - 如果你在一开始就生成代理，那么当 A 和 B 循环依赖时，B 注入的可能是一个“还没准备好的代理对象”，而不是 Spring 想要的半成品原始对象，这可能导致提前调用代理逻辑出错。

> 要回答清楚，得分两个层次：
>
> ------
>
> ### 1. Spring 目前的设计（为什么是在初始化阶段检查是否需要代理）
>
> Spring 的代理创建是由 **`BeanPostProcessor`** 完成的，特别是 `AbstractAutoProxyCreator`。
>
> - **时机**：`BeanPostProcessor#postProcessAfterInitialization` → 在 **Bean 实例化 + 属性注入完成** 之后。
> - **原因**：
>   1. 代理逻辑依赖于 **完整的 Bean 信息**（如 AOP 切点匹配、事务注解、@Async 等），这些元数据有时需要依赖于 **注入完成的上下文**。
>   2. 如果过早创建代理（比如在实例化时），可能缺少关键信息，导致匹配不准确。
>
> 所以 Spring 默认只能在初始化阶段决定是否包装成代理。
>  这就导致：
>
> - 实例化出来的“半成品” Bean 只能先放到 **三级缓存（ObjectFactory）**，以便循环依赖时返回。
> - 真正的代理替换要等到 `postProcessAfterInitialization` 才能发生。
>
> ------
>
> ### 2. 你提到的改造思路（在创建 Bean 阶段就检查代理需求）
>
> 理论上 **是可行的**，但是有几个前提和问题：
>
> ✅ **好处**：
>
> - 只需要二级缓存（不用 ObjectFactory 延迟代理），循环依赖时就能直接返回代理对象。
> - 可以减少一层缓存带来的复杂度。
>
> ❌ **问题**：
>
> 1. **元信息是否足够**
>
>    - 如果代理逻辑只依赖于 Bean 的 **类信息（Class）和 BeanDefinition**，那么确实可以在实例化阶段就决定是否要代理。
>    - 但是如果代理逻辑依赖于 **注解属性、BeanFactory 环境甚至依赖注入的结果**，那实例化前不一定能获取全。
>
>    👉 举个例子：
>
>    - `@Transactional` → 事务代理其实只需要方法和注解信息，理论上可以提前决定。
>    - `@Async` → 也只需要方法级别元数据，可以提前。
>    - 但是一些自定义 `BeanPostProcessor` 可能需要 Bean 初始化后的状态来决定是否代理，那就没法提前。
>
> 2. **和 Spring 的扩展机制耦合**
>     Spring 的 `BeanPostProcessor` 是插件化的，你没办法在实例化阶段就提前知道所有后置处理器的逻辑（除非硬改 Spring 逻辑）。
>
> 3. **生态兼容性**
>     绝大多数 Spring 生态依赖 `postProcessAfterInitialization` 的 AOP 扩展点，如果你改成实例化阶段，很多三方库可能行为异常。
>
> ------
>
> ### 3. 结论
>
> - **如果只考虑自己项目的代理逻辑**（例如只基于注解 `@Transactional` / `@Async`），完全可以在 **实例化阶段**就决定是否代理 → 这样就能省去三级缓存，直接二级缓存代理对象。
> - **但在 Spring 框架的通用机制里**，由于要支持各种 `BeanPostProcessor` 和生态兼容，所以必须保留三级缓存机制。

关键点在于：

- **提前代理只在循环依赖时才会触发**，属于一种「妥协」机制。

- 这种情况下，Spring 的假设是：

  > “虽然代理可能不完整，但比起抛出异常（无法解决循环依赖），还是先给你一个代理对象顶着用。”

也就是说，**正常路径必须保证代理逻辑是完整的（afterInitialization 阶段）**，但循环依赖是个例外。



> ### 4. 如果改成你提议的「实例化阶段就统一代理」
>
> 好处：
>
> - 逻辑更简单，不用三级缓存了，直接二级缓存存代理对象就行。
>
> 风险：
>
> 1. 有些 `BeanPostProcessor` 必须依赖 **初始化后的状态** 才能决定是否代理，你提前生成就会丢失正确性。
> 2. 提前代理会覆盖掉“初始化过程中可能动态改变 Bean 行为”的场景（比如某些框架会在 init 方法里加标记，然后某个 PostProcessor 根据标记决定是否做代理）。
> 3. Spring 生态（很多三方库）已经默认代理时机是 **afterInitialization**，你提前做可能导致兼容性问题。
>
> ------
>
> ### 5. 总结
>
> - **现在的设计**：
>   - 默认在 **初始化完成后** 再决定是否代理（保证准确性）。
>   - 循环依赖时，允许“提前代理”作为特例（保证可用性）。
> - **你的思路**：
>   - 如果场景单一、可控（比如只依赖注解信息，不依赖初始化状态），可以在实例化阶段就代理，从而简化缓存结构。
>   - 但通用框架层面，必须保留三级缓存，因为有太多扩展点需要 **初始化完成后的信息**。

Spring 怎么缓和 / 解决代理不完整这个问题

> **保留 `postProcessAfterInitialization` 流程**
>
> - 即使早期返回了代理，Spring 仍然会在该 Bean 后续完成初始化时走 `postProcessAfterInitialization`。
> - 但 `AbstractAutoProxyCreator` 内部会维护一个标记（例如 `earlyProxyReferences`），记录哪些 Bean 已经被提前代理过。`postProcessAfterInitialization` 在看到该标记后通常**不会再创建新的代理**，以避免重复包装（双重代理）。
...
==单例生命周期跟随IOC即使没有被外部引用也不会被gc处理，非单例Bean可以被gc处理，但要手动调用方法释放资源==
...
> ## 1. **Session（会话作用域）**
>
> - **特征**：同一个用户会话（Session）里共享一个 Bean，不同用户之间互不影响。
> - **应用场景**：用户相关的临时数据，例如购物车、用户偏好、表单向导。
>
> ### 示例：购物车
>
> ```java
...
> ```
>
> - 用户 A 的购物车里是 `["Apple", "Banana"]`
> - 用户 B 的购物车里是 `[]`
> - **说明：每个 Session 一个独立 Bean 实例**
>
> ------
>
> ## 2. **Application（应用作用域）**
>
> - **特征**：整个应用只有一个 Bean 实例，存活于 `ServletContext` 范围，所有请求共享。
> - **应用场景**：全局配置、缓存、统计信息。
>
> ### 示例：访问统计器
>
...
> ```
>
> - 所有用户的请求都会共享同一个 `AppStatistics` Bean。
> - 比如：
>   - 用户 A 访问 → 计数器=1
>   - 用户 B 访问 → 计数器=2
>   - 用户 C 访问 → 计数器=3
...
> ------
>
> ## 3. **WebSocket（WebSocket 作用域）**
>
> - **特征**：在每个 WebSocket 连接的生命周期内创建一个 Bean，每条连接独立。
> - **应用场景**：实时通信会话数据，比如聊天室里每个用户的上下文。
>
> ### 示例：聊天会话上下文
>
> ```java
...
> ```
>
> - 用户 A 建立 WebSocket 连接 → 拥有一个独立的 `ChatSessionContext`。
> - 用户 B 连接 → 拥有自己的 `ChatSessionContext`，互不干扰。
> - 当 WebSocket 关闭时，Bean 生命周期结束。
>
> ------
>
> ## 4. **Custom Scopes（自定义作用域）**
>
> - **特征**：你可以实现 `Scope` 接口，完全自定义 Bean 的生命周期。
> - **应用场景**：例如「每次业务事务」、「每次 API 调用」、「租户隔离」等特殊需求。
>
> ### 示例：定义一个“请求用户级别”的作用域
>
> 比如我们希望在 **当前请求的用户上下文** 中共享一个 Bean，而不是整个 Session：
>
> ```java
...
> ```
>
> Bean 的生命周期绑定到 **用户 ID**，而不是线程。
>
> 这样就可以灵活实现 **按用户维度** 的 Bean 生命周期，而不是绑定在标准的 Session 或 Application 上。
>
> ------
>
> 📌 总结对比：
>
> - **Session**：每个用户会话独立（如购物车）。
> - **Application**：全应用共享一个（如全局统计器）。
> - **WebSocket**：每个 WebSocket 连接独立（如聊天上下文）。
> - **Custom**：自己定义，比如「每个租户」「每个事务」的作用域。
...
你的Bean类可以==实现org.springframework.beans.factory.InitializingBean和org.springframework.beans.factory.DisposableBean接==口，并分别实现==afterPropertiesSet和destroy==方法。
...
> Spring 框架提供了丰富的扩展点，允许开发者在容器的不同生命周期阶段插入自定义逻辑。以下是 **10 个核心扩展点**的详细说明和代码示例：
>
> ------
>
> ### **1. BeanFactoryPostProcessor**
>
> **作用**：在 Bean 定义加载后、实例化前修改 Bean 的定义（如修改属性、作用域）。
>  ​**​典型场景​**​：动态修改配置、替换占位符。
>  ​**​示例​**​：替换所有 Bean 的 `scope` 为原型模式
>
> ```
> @Component
...
>             definition.setScope(BeanDefinition.SCOPE_PROTOTYPE); // 强制改为原型
>         }
>     }
> }
> ```
...
> }
> 
> // 使用：@Import(EnvAwareImportSelector.class)
> ```
>
> ------
>
...
> }
> 
> // 注册：WebMvcConfigurer.addInterceptors()
> ```
>
> ------
>
...
>         System.out.println("容器已刷新，初始化缓存...");
>     }
> }
> ```
>
...
>
> **作用**：所有单例 Bean 初始化完成后执行。
>  ​**​典型场景​**​：依赖检查、启动后操作。
>  ​**​示例​**​：验证所有 Service 是否注入成功
>
> ```
> @Component
> public class DependencyChecker implements SmartInitializingSingleton {
>     @Autowired
>     private List<MyService> services; // 收集所有MyService
> 
>     @Override
>     public void afterSingletonsInstantiated() {
>         if (services.isEmpty()) {
>             throw new IllegalStateException("未找到任何MyService实现！");
>         }
>     }
> }
> ```
...
> | **扩展点**                   | **触发时机**               | **典型应用**           |
> | ---------------------------- | -------------------------- | ---------------------- |
> | `BeanFactoryPostProcessor`   | Bean 定义加载后            | 动态修改 Bean 配置     |
> | `BeanPostProcessor`          | Bean 初始化前后            | AOP 代理、监控         |
> | `PropertySource`             | 环境准备阶段               | 自定义配置源           |
> | `ImportSelector`             | 配置类加载时               | 条件化模块导入         |
> | `HandlerInterceptor`         | HTTP 请求处理前后          | 权限控制、日志记录     |
> | `@ControllerAdvice`          | 控制器方法执行后           | 全局异常处理           |
> | `@Conditional`               | 自动配置条件判断           | Starter 库开发         |
> | 自定义注解 + AOP             | 方法调用时                 | 声明式事务、日志       |
