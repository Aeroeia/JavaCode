...
5. **方法区（元空间）**：在 JDK 1.8 及以后的版本中，==方法区被元空间取代，使用本地内存==。用于存储已被虚拟机==加载的类信息、常量、~~静态变量~~==等数据。虽然方法区被描述为堆的逻辑部分，但有 “非堆” 的别名。方法区可以选择不实现垃圾收集，内存不足时会抛出 OutOfMemoryError 异常。
   ...
   **方法区（JDK8 后 → 元空间 Metaspace）中存放的内容**

   元空间（Metaspace）中只保存 **类元数据**，包括：

   - 类的结构信息（字段、方法、接口等的描述）
   - 方法字节码、运行时常量表（注意是 class 文件中的常量，不是运行时对象）
   - 方法的符号引用、注解、泛型签名等元信息
...
- `ThreadLocalMap` 的 key 使用弱引用，避免内存泄漏。
  ...
  >
  > - 即使我们 `local = null`；
  > - ThreadLocalMap 仍然持有对这个 ThreadLocal 的强引用；
  > - 这个 ThreadLocal 永远不会被 GC 回收；
  > - 它所在的线程（尤其是线程池线程）可能长期存在；
  > - 🚨 从而导致**严重内存泄漏**（整个 Map 的 entry 永远清不掉）。
  >
  > 所以为了避免这个问题：
  >
  > > key 改成弱引用（`WeakReference<ThreadLocal<?>>`）。
...
| 部分              | 子内容                         | 说明                               | 适用对象        |
| ----------------- | ------------------------------ | ---------------------------------- | --------------- |
| **Mark Word**     | 锁标志位（2 bit）              | 决定 Mark Word 当前存储内容类型    | 普通对象 / 数组 |
|                   | 偏向标志（1 bit）              | 1 = 可偏向，0 = 不可偏向           | 普通对象 / 数组 |
|                   | 对象年龄（4 bit）              | GC 使用的分代年龄                  | 普通对象 / 数组 |
|                   | **无锁状态：对象 hashCode**    | identityHashCode（25 bit）         | 普通对象 / 数组 |
|                   | **偏向锁：线程 ID + Epoch**    | 记录偏向线程，大部分占用 Mark Word | 普通对象 / 数组 |
|                   | **轻量级锁：指向 Lock Record** | Lock Record 在持锁线程栈上         | 普通对象 / 数组 |
|                   | **重量级锁：指向 Monitor**     | Monitor 维护 EntryList/WaitSet     | 普通对象 / 数组 |
|                   | **GC 状态信息**                | GC 标记时可能暂存一些信息          | 普通对象 / 数组 |
| **Klass Pointer** | 指向 Klass 元数据              | 类信息（字段、方法表、元数据）     | 普通对象 / 数组 |
| **Array Length**  | 数组长度（4 byte）             | 仅数组对象有                       | 数组对象        |

==对象创建时一开始是符号引用 在元空间中检查是否有这个类的InstanceKlass信息 如果发现类还没有加载 则进行类加载 如果加载好了则将进行对象创建 将对象头的Klass指向InstanceKlass== 
...
==对象的创建是基于元空间的类信息 Class和Method对象用于反射创建对象以及访问变量等==

==符号引用（存于常量池中）指的是这个类的全包名 而直接引用是这个类的元数据 类解析后会被类加载器缓存==
...
**Initial Mark = 快速找到 GC Roots 直接引用的老年代对象**

**Concurrent Mark = 并发标记写屏障下记录的对象 标记的是快照下的对象，标记大部分对象**

**Remark = 修正并发阶段遗漏（通过屏障记录），保证可达性完全准确**

==写前屏障是为了记录变更 扫描到的时候把这个记录加到一个集合中==

- **新创建的对象**
  - 并发标记阶段新创建的对象可能未被标记
  - 通过 **写屏障 + Card Table / Remembered Set** 被记录
  - Remark 扫描这些记录，递归标记可达对象
- **并发期间修改引用的对象**
  - 已标记的对象引用发生变化（指向新对象或断开引用）
  - Remark 阶段需要检查这些引用的变化，更新标记位

✅ 也就是说，Remark 阶段是 **对并发阶段已有标记 + 新对象 + 修改引用对象的修正**

| 特性     | Concurrent Mark                      | Remark                                         |
| -------- | ------------------------------------ | ---------------------------------------------- |
| 停顿     | 并发                                 | Stop-the-World（短）                           |
| 遍历对象 | 全堆（大部分对象）                   | 主要处理遗漏对象/修改引用的对象                |
| 遍历路径 | 从 GC Roots 出发沿引用链遍历         | 从 GC Roots 出发，主要关注并发阶段遗漏的脏对象 |
| 目的     | 标记大部分对象，收集 Region 垃圾比例 | 修正遗漏，保证标记准确，为 Evacuation 准备     |
