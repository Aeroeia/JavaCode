...
| 部分                          | 说明                                                   | 大小（字节）                                |
| ----------------------------- | ------------------------------------------------------ | ------------------------------------------- |
| **对象头（Object Header）**   | 存放对象运行时信息，例如哈希码、GC分代年龄、锁标志等。 | 8 字节（Mark Word） + 4 或 8 字节（类指针） |
| **实例数据（Instance Data）** | 对象的字段（即成员变量）。                             | 由字段类型决定                              |
| **对齐填充（Padding）**       | 让对象大小是 8 的倍数（JVM 内存对齐要求）。            | 0~7 字节                                    |
...
>
> **核心思想**：**定流程，变步骤**。老爸规定做作业的流程，具体每一步怎么做，儿子自己实现。
>
> **生活例子**：
>  老爸规定做作业的流程必须是：
>
...
>     
>     // 可变步骤：抽象方法，子类必须实现
>     protected abstract void doYourWork();
> }
> 
> // 大儿子做数学作业
> public class MathHomework extends HomeworkTemplate {
>     @Override
>     protected void doYourWork() {
>         System.out.println("我在做数学作业！");
>     }
> }
> 
> // 小儿子做语文作业  
> public class ChineseHomework extends HomeworkTemplate {
>     @Override
>     protected void doYourWork() {
...
> public class Test {
>     public static void main(String[] args) {
>         HomeworkTemplate math = new MathHomework();
>         math.doHomework(); // 输出：打开书包 → 我在做数学作业！ → 收拾书包
>         
>         HomeworkTemplate chinese = new ChineseHomework();
>         chinese.doHomework(); // 输出：打开书包 → 我在写作文！ → 收拾书包
>     }
> }
> ```
>
...
>
> **核心思想**：**穿衣服，一层层加功能**。给一个人穿衣服，可以穿T恤、穿衬衫、穿西装，层层装饰。
>
> **生活例子**：
>  一个人：`Person`
>  装饰他：`TShirtDecorator`、`SuitDecorator`、`TieDecorator`
>
> **代码例子**：
>
> ```
...
> 
> // 具体组件：被装饰的人
> public class NakedPerson implements Person {
>     @Override
>     public void dress() {
>         System.out.print("裸体的人");
>     }
> }
> 
> // 装饰器抽象类
> public abstract class ClothesDecorator implements Person {
>     protected Person person; // 持有被装饰的对象
>     
...
>     @Override
>     public void dress() {
>         person.dress(); // 调用被装饰对象的方法
>     }
> }
> 
> // 具体装饰器：T恤
> public class TShirtDecorator extends ClothesDecorator {
...
>     @Override
>     public void dress() {
>         super.dress(); // 先调用被装饰对象的方法
>         System.out.print(" + 穿T恤"); // 然后添加新功能
>     }
> }
> 
> // 具体装饰器：西装
...
> public class Test {
>     public static void main(String[] args) {
>         Person me = new NakedPerson(); // 裸体的人
>         me = new TShirtDecorator(me);  // 给他穿T恤
>         me = new SuitDecorator(me);    // 再穿西装
>         
>         me.dress(); // 输出：裸体的人 + 穿T恤 + 穿西装
>     }
> }
> ```
>
...
>
> **核心思想**：**微信订阅号**。你关注了一个公众号，它一发文章，你就收到通知。
>
> **生活例子**：
>
> - 被观察者：微信公众号
...
>     void update(String article); // 收到更新通知
> }
> 
> // 具体观察者：用户
...
>         
>         account.addSubscriber(zhangsan); // 张三关注
>         account.addSubscriber(lisi);     // 李四关注
>         
>         account.publishArticle("设计模式详解"); // 发布文章
>         // 输出：
>         // 张三 收到新文章：设计模式详解
>         // 李四 收到新文章：设计模式详解
>     }
...
>
> **核心思想**：**转接头**。你的中国插头不能直接插欧洲插座，需要一个转接头适配。
>
> **生活例子**：
>
> - 中国插头：`ChinesePlug`
...
>     public void insert() {
>         System.out.println("中国插头插入");
>     }
> }
> 
> // 适配器：转接头
> public class PlugAdapter implements EuropeanSocket {
>     private ChinesePlug chinesePlug; // 持有被适配对象
>     
>     public PlugAdapter(ChinesePlug plug) {
>         this.chinesePlug = plug;
>     }
>     
>     @Override
>     public void plugIn() {
>         chinesePlug.insert(); // 调用被适配对象的方法
>         System.out.println("通过转接头适配到欧洲插座");
>     }
> }
> 
> // 测试
> public class Test {
>     public static void main(String[] args) {
>         ChinesePlug myPlug = new ChinesePlug(); // 我的中国插头
>         
>         EuropeanSocket adapter = new PlugAdapter(myPlug); // 转接头
>         
>         adapter.plugIn(); // 成功插入欧洲插座
>         // 输出：
>         // 中国插头插入
>         // 通过转接头适配到欧洲插座
>     }
...
> | 模式         | 核心思想               | 简单例子                         |
> | ------------ | ---------------------- | -------------------------------- |
> | **模板方法** | 定流程，变步骤         | 老爸定做作业流程，儿子自己写作业 |
> | **装饰器**   | 层层包装，动态添加功能 | 给人穿衣服，T恤→衬衫→西装        |
> | **观察者**   | 一对多依赖，自动通知   | 微信公众号发文，所有粉丝收到通知 |
> | **适配器**   | 转换接口，兼容不同     | 中国插头用转接头插欧洲插座       |
>
> 这四种模式都非常实用，理解后对你的编程设计能力会有很大提升！
